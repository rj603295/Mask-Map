...
const upDataCounty = (importData) => {
  ...
  importData.forEach((item) => {
    str += `<option value="${item.CityName}">
...
function updateTown () {
  ...
  for (let i = 0; i < area.length; i++) {
    if (area[i].CityName === county.value) {
      newTown.push({
        town: area[i].AreaList
      })
    }
  };
  let str = ''
...
function updateList () {
  const countySelect = county.value
  const townSelect = countyTown.value
...
// 鄉鎮change功能
countyTown.addEventListener('change', updateList, false)
...
function getMarker () {
  ...
  for (let i = 0; array.length > i; i++) {
    ...
    <p>${array[i].properties.address}</p>
      <div class="container">
        <div class="row justify-content-around">
          <div class="col-6 p-1">
            <div class="bg-adult py-2 rounded-pill text-white mask-size">
              <p class="font-size text-center m-0 px-1">成人口罩</p>
              <p class="text-center m-0">${array[i].properties.mask_adult}</p>
            </div>
          </div>
          <div class="col-6 p-1">
            <div class="bg-child py-2 rounded-pill text-white mask-size">
              <p class="font-size text-center m-0 px-1">兒童口罩</p>
              <p class="text-center m-0">${array[i].properties.mask_child}</p>
            </div>
          </div>
        </div>
      </div>`))
...
function renderDay () {
  ...
  const _chineseDay = DayChange(_day)
  const _thisDay = _date.getFullYear() + '-' + (_date.getMonth() + 1) + '-' + _date.getUTCDate()
...
function targetPharmacy (e, targetData) {
  ...
  const adult = e.currentTarget.dataset.adult
          ...
          <div class="container">
            <div class="row justify-content-around">
              <div class="col-6 p-1">
                <div class="bg-adult py-2 rounded-pill text-white mask-size">
                  <p class="font-size text-center m-0 px-1">成人口罩</p>
                  <p class="text-center m-0">${adult}</p>
                </div>
              </div>
              <div class="col-6 p-1">
                <div class="bg-child py-2 rounded-pill text-white mask-size">
                  <p class="font-size text-center m-0 px-1">兒童口罩</p>
                  <p class="text-center m-0">${child}</p>
                </div>
              </div>
            </div>
          </div>`).openPopup()
